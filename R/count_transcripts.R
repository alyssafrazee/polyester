...
#' @param fasta TRUE if \code{f} is a fasta file; FALSE if \code{f} is a GTF file
#' @param identifier if \code{f} is a GTF file, how are transcripts identified in the
#'   attributes field (9th column) of the file? Default \code{transcript_id}.
#' @param attrsep if \code{f} is a GTF file, how are attributes separated in the attributes field 
#'   (9th column) of the file? Default "; ". 
#' @export
...
#'
count_transcripts = function(f, fasta=TRUE, identifier='transcript_id', attrsep="; "){
    flength = nchar(f)
    if(fasta){
    ...
    gtf_info = read.table(f, sep="\t", as.is=TRUE, quote="", header=FALSE, comment.char="#", 
        nrows= -1, colClasses=c(rep('NULL', 8), 'character'))
